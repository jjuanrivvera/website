...
export const ui = {
  en: {
    ...
    'projects.garminctl.description':
      'Command-line interface for Garmin Connect health and activity data: body composition, sleep, heart rate, stress, and body battery, plus the full Connect endpoint surface and a raw API escape hatch. Built in Go around go-garmin, it fixes the recurring authentication failure by refreshing the OAuth2 token before every request and persisting it to the OS keyring. Includes named profiles for several accounts, scriptable table/json/yaml/csv output, an MCP server, and an agent guard. Built with the cliwright playbook.',
  ...
  es: {
    ...
    'projects.garminctl.description':
      'Interfaz de línea de comandos para los datos de salud y actividad de Garmin Connect: composición corporal, sueño, frecuencia cardíaca, estrés y body battery, más toda la superficie de endpoints de Connect y una vía directa a la API. Construida en Go sobre go-garmin, resuelve el fallo de autenticación recurrente refrescando el token OAuth2 antes de cada petición y guardándolo en el keyring del sistema. Incluye perfiles con nombre para varias cuentas, salida automatizable en tabla/json/yaml/csv, un servidor MCP y un guard de agentes. Construida con el playbook de cliwright.',
    'projects.canvasKitLaravel.description':
  ...
  pt: {
    ...
    'projects.garminctl.description':
      'Interface de linha de comando para os dados de saúde e atividade do Garmin Connect: composição corporal, sono, frequência cardíaca, estresse e body battery, além de toda a superfície de endpoints do Connect e um acesso direto à API. Construída em Go sobre o go-garmin, resolve a falha de autenticação recorrente atualizando o token OAuth2 antes de cada requisição e guardando-o no keyring do sistema. Inclui perfis nomeados para várias contas, saída automatizável em tabela/json/yaml/csv, um servidor MCP e um guard de agentes. Construída com o playbook do cliwright.',
    'projects.canvasKitLaravel.description':
