...
export const ui = {
  en: {
    ...
    'projects.garminctl.description':
  ...
  es: {
    ...
    'projects.edc.description':
      'Un canal agnóstico al transporte para Claude Code que convierte eventos externos en turnos del agente. Abre un endpoint local /inject protegido con secret, así que cualquier proceso (un cron, un watcher de archivos, otro agente) puede hacer POST de un evento y verlo aparecer como un turno en una sesión en vivo. Sin polling: la sesión duerme gratis y reacciona solo cuando de verdad pasa algo. Los eventos inyectados entran marcados como data de sistema, no como órdenes, que es la defensa integrada contra prompt injection. Implementado como servidor MCP stdio con archivos de estado por sesión y manejo de procesos multiplataforma.',
    'projects.slackctl.description':
      'Herramienta de línea de comandos y stream de eventos en vivo para Slack, construida con Go. Se conecta por Socket Mode o RTM, guarda cada evento en un store local de SQLite y añade búsqueda, tail y estadísticas sobre ese historial. Las credenciales viven en el keyring del sistema, la salida es automatizable (tabla/json/yaml con un filtro --jq) e incluye un servidor MCP y un guard de agentes para que agentes de IA operen Slack de forma segura. Construida con el playbook de cliwright.',
    'projects.wootctl.description':
      'Interfaz de línea de comandos para toda la API de Chatwoot (144 operaciones): conversaciones, contactos, agentes, equipos, bandejas de entrada, reportes y automatizaciones, más backup/restore y sincronización entre instancias. Construida en Go con perfiles con nombre, autenticación en el keyring, salida automatizable en tabla/json/yaml, un servidor MCP y un guard de agentes para que agentes de IA operen Chatwoot de forma segura. Construida con el playbook de cliwright.',
    'projects.lsqueezy.description':
    ...
    'projects.garminctl.description':
      'Interfaz de línea de comandos para los datos de salud de Garmin Connect (composición corporal, sueño, frecuencia cardíaca, estrés, body battery), más el registro completo de endpoints y una vía directa a la API. Construida en Go sobre go-garmin, que ya es una CLI de Garmin más amplia; garminctl añade almacenamiento de tokens en el keyring del sistema en vez de un archivo de sesión en texto plano, perfiles con nombre para varias cuentas, importación de tokens garth existentes, salida automatizable en tabla/json/yaml/csv, un servidor MCP y un guard de agentes. Construida con el playbook de cliwright.',
    'projects.canvasKitLaravel.description':
  ...
  pt: {
    ...
    'projects.edc.description':
      'Um canal agnóstico ao transporte para o Claude Code que transforma eventos externos em turnos do agente. Abre um endpoint local /inject protegido por secret, então qualquer processo (um cron, um watcher de arquivos, outro agente) pode fazer POST de um evento e vê-lo aparecer como um turno numa sessão ao vivo. Sem polling: a sessão dorme de graça e reage apenas quando algo real acontece. Os eventos injetados chegam marcados como dados de sistema, não como comandos, que é a defesa integrada contra prompt injection. Implementado como servidor MCP stdio com arquivos de estado por sessão e tratamento de processos multiplataforma.',
    'projects.slackctl.description':
      'Ferramenta de linha de comando e stream de eventos ao vivo para o Slack, construída com Go. Conecta-se por Socket Mode ou RTM, grava cada evento num store local de SQLite e adiciona busca, tail e estatísticas sobre esse histórico. As credenciais ficam no keyring do sistema, a saída é automatizável (tabela/json/yaml com um filtro --jq) e inclui um servidor MCP e um guard de agentes para que agentes de IA operem o Slack com segurança. Construída com o playbook do cliwright.',
    'projects.wootctl.description':
      'Interface de linha de comando para toda a API do Chatwoot (144 operações): conversas, contatos, agentes, equipes, caixas de entrada, relatórios e automações, além de backup/restore e sincronização entre instâncias. Construída em Go com perfis nomeados, autenticação no keyring, saída automatizável em tabela/json/yaml, um servidor MCP e um guard de agentes para que agentes de IA operem o Chatwoot com segurança. Construída com o playbook do cliwright.',
    'projects.lsqueezy.description':
    ...
    'projects.garminctl.description':
      'Interface de linha de comando para os dados de saúde do Garmin Connect (composição corporal, sono, frequência cardíaca, estresse, body battery), além do registro completo de endpoints e um acesso direto à API. Construída em Go sobre o go-garmin, que já é uma CLI de Garmin mais ampla; o garminctl adiciona armazenamento de tokens no keyring do sistema em vez de um arquivo de sessão em texto plano, perfis nomeados para várias contas, importação de tokens garth existentes, saída automatizável em tabela/json/yaml/csv, um servidor MCP e um guard de agentes. Construída com o playbook do cliwright.',
    'projects.canvasKitLaravel.description':
